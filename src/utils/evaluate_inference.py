...
def evaluate_answer_covid(test_ds, path_inference):
    ...
    bleu_corpus = get_bleu(inference_answers, true_answers)    
    
    df["question"] = test_ds['question']
    df["id"] = test_ds['id']
    df["true_answer"] = true_answers
    #df["rougeL_f1"] = scores_rouge
    #df["bleu"] = bleu_scores
    #df = df[['id', 'question', 'true_answer', 'inference', 'rougeL_f1', 'bleu']]
    df = df[['id', 'question', 'true_answer', 'inference']]
    df.to_csv(path_inference, index=False)
    save_final_result(path_inference, avg_score, bleu_corpus)
...
def evaluate_answer_clapnq(test_ds, path_inference):
    ...
    df = df[['id', 'question', 'true_answer', 'inference']]
    df.to_csv(path_inference, index=False)
    save_final_result(path_inference, avg_score, bleu_corpus, accuracy)
    print(f"Results saved in: {path_inference}")
